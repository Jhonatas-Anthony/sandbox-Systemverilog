...
// Este testbench foi criado para demonstrar o comportamento dos operadores lógicos (AND, OR, NOT)
// e dos operadores de concatenação e replicação.
module tb_operators;

  // Declaração de variáveis do tipo 'logic'.
  // Cada variável ('a', 'b', 'c') terá 4 bits de largura, permitindo valores de 0 a 15.
  logic [3:0] a, b, c; // -- Definindo 3 variáveis 'a', 'b' e 'c', cada uma com 4 bits.

  // Bloco 'initial'.
  // Este bloco é executado apenas uma vez no início da simulação.
  // Ele é ideal para inicializar sinais, aplicar estímulos e verificar a lógica em um testbench.
  initial
    begin // Início do bloco 'initial' sequencial.
      ...
      $display("Conc de 'a' e 'b' - %0b", {a,b}); // -- Saída esperada: "Conc de 'a' e 'b' - 01101001"

      // Operador de Replicação: {quantidade{padrão}}
      // Repete um vetor de bits (ou literal) um número especificado de vezes.
      // {4{2'b01}} significa que o padrão 2'b01 será repetido 4 vezes.
      // Padrão: 2'b01
      // Repetição 4 vezes: 01 01 01 01 (um vetor de 8 bits)
      $display("Conc e repeticao de um padrao - %0b", {4{2'b01}}); // -- Saída esperada: "Conc e repeticao de um padrao - 01010101"

      // Finaliza a simulação.
      // Garante que o simulador pare após a execução de todas as operações do testbench.
      $finish;
