...
// Este testbench será usado para simular e verificar o comportamento dos operadores de bit a bit e de deslocamento.
module tb_operators;

  // Declaração de variáveis do tipo 'logic'.
  // 'logic' em SystemVerilog é um tipo de dado flexível, adequado para variáveis de testbench.
  // [3:0] indica que cada variável tem 4 bits de largura, do bit mais significativo (3) ao menos significativo (0).
  logic [3:0] a, b; // -- Definindo 2 variáveis 'a' e 'b', cada uma com 4 bits.

  // Bloco 'initial'.
  // Este bloco de código é executado uma única vez no início da simulação (tempo 0).
  // É aqui que definimos os valores iniciais para as nossas variáveis e aplicamos os operadores para observar os resultados.
  initial
    begin // Início do bloco 'initial' sequencial.
      ...
      // É uma boa prática usar $finish no final de um testbench para encerrar a simulação
      // após a execução de todos os testes e observações.
      $finish;
